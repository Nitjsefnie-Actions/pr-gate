...
if fixtures.get('expected_token') and os.environ.get('GH_TOKEN') != fixtures['expected_token']:
    finish(401, {'message': 'wrong action token'})
...
if endpoint != 'markdown':
    ...
    if not endpoint.startswith(prefix):
        unsupported()
    endpoint = 'repos/owner/repo/' + endpoint[len(prefix):]
    number = fixtures.get('pull_number', '99')
    endpoint = endpoint.replace('/pulls/' + number, '/pulls/99')
    endpoint = endpoint.replace('/issues/' + number + '/', '/issues/99/')
template_path = quote(fixtures.get('template_path', '.github/PULL_REQUEST_TEMPLATE.md'), safe='/')
base = fixtures.get('pull', {}).get('base', {}).get('sha', '')
if method == 'GET' and endpoint == f'repos/owner/repo/contents/{template_path}?ref={base}':
    finish(200, {'type': 'file', 'encoding': 'base64',
                 'content': base64.b64encode(fixtures['template'].encode()).decode()})
